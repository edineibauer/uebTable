...
if ($setor === "admin" || empty($permissoes[$entidade]) || (isset($permissoes[$entidade]['read']) && $permissoes[$entidade]['read'])) {
    ...
    $sql->exeCommand("SELECT COUNT(*) as total FROM " . PRE . $entidade . (!isset($permissoes[$entidade]["explore"]) || !$permissoes[$entidade]["explore"] ? " WHERE system_id = {$_SESSION["userlogin"]["system_id"]}" : ""));
